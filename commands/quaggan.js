...
exports.run = (client, message) => {
    ...
    snekfetch.get("https://api.guildwars2.com/v2/quaggans").then( r => {
        if (r.status.statusCode < 200 || r.status.statusCode > 299 || r.ok == false) return message.channel.send("API is on :fire:, please wait for the :fire_engine: to arrive.");

        message.channel.send("Coo!\nhttps://static.staticwars.com/quaggans/"+r.body[Math.floor(Math.random() * r.body.length)]+".jpg");
    });
};
exports.conf = {
  enabled: true,
