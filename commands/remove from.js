...
exports.run = (client, message, params) => {
    ...
    if (message.mentions.users.size < 1){//If there is no mention				
       functions.getUserByName(badUsername, message, function haveId(badUser){//grab the user id from the displayname
            if(badUser === false) return message.channel.send("Could not find user, please type their display name exactly as seen on discord.");
            if(json.indexOf(badUser.id) === -1) return message.channel.send("User was not found in the list, constact Daro is this is unexpected.");
            
            json = json.filter(function(e) { return e !== badUser.id; });
            fs.writeFile(listPath, JSON.stringify(json, null, 4), function (err){
                if(!err) return message.channel.send("User has been removed from the list.");
                
                message.channel.send("There was an issue in removing the user from the list.");
