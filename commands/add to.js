...
exports.run = (client, message, params) => {
    ...
    if(!listChoice) return message.channel.send("Command was not valid.");
    ...
    if (message.mentions.users.size < 1){//If there is no mention
        if(!badUsername) return message.channel.send("No user was specified, did you mean to query !GET <list>?");
        ...
        functions.getUserByName(badUsername, message, function haveId(badUser){//grab the user id from the displayname
            if(badUser === false) return message.channel.send("Could not find user, please type their display name exactly as seen on discord or mention them with \`@\`."); 
            
            if(json.indexOf(badUser.id) !== -1) return message.channel.send("User is already in the list, if this is unexpected, contact Daro.");
            
            json.push(badUser.id);
            fs.writeFile(listPath, JSON.stringify(json, null, 4), function (err){
                if(err){
                    message.channel.send("There was an issue in adding the user to the list.");
                    return console.log("Had issue writting file. "+err.message);
                }
                else{message.channel.send("User has been added to the list.");}
            });
        });
    }
